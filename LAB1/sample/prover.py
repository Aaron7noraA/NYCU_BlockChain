...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        # record the neighbor hash
        if level_pos % 2 == 0:
            path.append(state[level_pos+1])
        else:
            path.append(state[level_pos-1])
            
        tmp_list.clear() # clear the list, making new record
        for idx in range(0, len(state), 2):
            
            # making new hash list for the next iterations
            hash_tag = hash_internal_node(state[idx], state[idx + 1])
            tmp_list.append(hash_tag)
            
            # new pos for the next iterations
            if (level_pos // 2) * 2 == idx:
                level_pos = idx // 2
        
        state.clear()
        state = [i for i in tmp_list]
